  ...
  try {
    ...
    if (fullName !== undefined) {
      if (!fullName || fullName.trim().length === 0) {
        ...
        validation.addError("fullName", "Full name must be at least 2 characters long");
      }
    }
    ...
    if (email !== undefined) {
      if (!email || email.trim().length === 0) {
        ...
        if (existingUser && existingUser.id !== req.userId) {
          validation.addError("email", "Email is already taken by another user");
        }
      }
    }
    ...
    if (password) {
      if (!confirmPassword) {
        validation.addError("confirmPassword", "Password confirmation is required");
      } else if (password !== confirmPassword) {
        validation.addError("confirmPassword", "Passwords do not match");
      } else if (password.length < 8) {
        validation.addError("password", "Password must be at least 8 characters long");
      } else if (!/[A-Z]/.test(password)) {
        validation.addError("password", "Password must contain at least one uppercase letter");
      } else if (!/[a-z]/.test(password)) {
        validation.addError("password", "Password must contain at least one lowercase letter");
      } else if (!/[0-9]/.test(password)) {
        validation.addError("password", "Password must contain at least one number");
      }
    }
